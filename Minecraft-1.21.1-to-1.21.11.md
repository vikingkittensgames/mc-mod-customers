...
| RGB GUI colors such as `0xFFFFFF` render as opaque white | GUI colors are interpreted as ARGB, making `0xFFFFFF` transparent | Use `0xFFFFFFFF` or `-1` for opaque white GUI text. |
...
| Generic deferred block registration can construct blocks from a supplier | Block construction requires registry-aware properties with the block ID set | Use `DeferredRegister.Blocks.registerBlock(...)` so NeoForge applies the block registry key to `BlockBehaviour.Properties`. |
...
| `BlockEntityRenderer<T>` renders block entities directly through `render(...)` | `BlockEntityRenderer<T, S>` extracts a `BlockEntityRenderState` and submits it through `SubmitNodeCollector` | Keep item layout calculations shared, but resolve `ItemStackRenderState` values during extraction and submit them from the renderer's `submit(...)` implementation. |
| NeoForge provides `BlockStateProvider`, `BlockModelBuilder`, and `ExistingFileHelper` | The former NeoForge model-generator classes are unavailable | Generate custom layered block, blockstate, and legacy item-model JSON through a focused `DataProvider`. |
| An item model under `assets/<namespace>/models/item` is sufficient | Items require an item definition under `assets/<namespace>/items` that references the model | Keep legacy item models for geometry and transforms, and add item definitions using the `minecraft:model` item-model type. |
| A single `GatherDataEvent` exposes `includeClient()` and `includeServer()` | Client and server data generation use `GatherDataEvent.Client` and `GatherDataEvent.Server` | Register separate client and server gather-data listeners. |
| The `data()` run type generates client and server resources together | The `clientData()` and `serverData()` run types generate their respective resources separately | Keep an aggregate `runData` task that runs both version-specific data tasks in order. |
| One data run owns all generated output and its cache | Client and server data runs remove files not produced by their own provider set when sharing an output directory | Generate into separate build directories and merge both results into `src/generated/resources` after both runs complete. |
| Custom recipe providers extend `RecipeProvider` and receive `PackOutput` plus registries | Custom recipe providers extend `RecipeProvider.Runner` and create a registry-aware `RecipeProvider` | Build recipes through the provider's `items` lookup and parameterless `buildRecipes()` method. |
| A recipe can explicitly save under an ID matching its result item's ID | `RecipeBuilder.save(output, id)` rejects an explicitly supplied default ID | Call `save(output)` when the recipe ID should match the registered result item. |
| Generated shaped-recipe keys use objects such as `"I": { "item": "minecraft:iron_ingot" }` | Generated shaped-recipe keys use shorthand strings such as `"I": "minecraft:iron_ingot"` | Keep generated-resource tests aligned with each version's serialized recipe format. |
