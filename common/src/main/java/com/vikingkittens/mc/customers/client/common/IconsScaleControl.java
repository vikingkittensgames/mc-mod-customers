...
public class IconsScaleControl extends AbstractWidget {
    ...
    public void onClick(double mouseX, double mouseY) {
        int iconIndex = Mth.clamp(((int)mouseX - getX()) / iconWidth, 0, iconCount - 1);
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        for (int iconIndex = 0; iconIndex < iconCount; iconIndex++) {
            ...
            ResourceLocation icon = remaining >= 1.0F ? fullIcon : remaining >= increment ? halfIcon : emptyIcon;
            graphics.blit(
                    icon,
    ...
    private float clamp(float value) {
        return Mth.clamp(Math.round(value / increment) * increment, minimum, maximum);
    }
