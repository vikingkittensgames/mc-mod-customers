...
class ValueInputDataReader implements DataReader {
    ...
    public List<String> getStrings(String key) {
        return input.listOrEmpty(key, Codec.STRING).stream().toList();
    }
    ...
    public List<ItemStack> getItemStacks(String key) {
        return input.listOrEmpty(key, ItemStack.OPTIONAL_CODEC).stream().toList();
    }
    ...
    public List<DataReader> getChildren(String key) {
        return input.childrenListOrEmpty(key).stream().map(ValueInputDataReader::new).map(DataReader.class::cast).toList();
    }
}
