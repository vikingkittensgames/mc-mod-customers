...
a topper block like carpet or candle:
...
avoid block. By default, dirt, grass blocks, sand, and snow blocks cannot be used as avoid
blocks. These blocks are common around builds and can otherwise cause unexpected customer
routing when an avoid block was not deliberately set up.
...
`data/customers/tags/blocks/can_avoid.json` in their resources. Keep `"replace": false` so
...
Save that second example as `data/customers/tags/blocks/can_not_avoid.json`.
